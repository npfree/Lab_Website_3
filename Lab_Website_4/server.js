...
app.post('/home/pick_color', function(req, res) {
	...
	var insert_statement = 'INSERT * INTO favorite_colors;'; // Write a SQL statement to insert a color into the favorite_colors table
...
app.get('/team_stats', function(req, res) {
	...
	db.task('load-team-stats', task => {
        return task.batch([
            task.any(games),
            task.any('SELECT COUNT(*) AS winning_games FROM football_games WHERE home_score>visitor_score'),
            task.any('SELECT COUNT(*) AS losing_games FROM football_games WHERE home_score<visitor_score')
        ]);
    })
    .then(stats => {
    	res.render('pages/team_stats',{
				my_title: "Team Stats for 2020 Season",
				season_games: stats[0], // Return games
				totwins: stats[1][0].winning_games, // Return total wins
				totlosses: stats[2][0].losing_games // Return total losses
			})
    })
    .catch(err => {
            console.log('error', err);
